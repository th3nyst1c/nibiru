...
func TestExecuteFullLiquidation(t *testing.T) {
	...
	for name, testCase := range testCases {
		...
		t.Run(name, func(t *testing.T) {
			...
			var err error
			err = testutil.FundAccount(nibiruApp.BankKeeper, ctx, traderAddr,
			...
			assert.NoError(t, testutil.FundModuleAccount(
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for i, pos := range tc.positions {
				...
				require.NoError(t, testutil.FundModuleAccount(app.BankKeeper, ctx, types.VaultModuleAccount, sdk.NewCoins(sdk.NewInt64Coin(pos.Pair.QuoteDenom(), 1))))
