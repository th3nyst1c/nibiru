...
func TestNewPool(t *testing.T) {
	...
	err := testutil.FundAccount(app.BankKeeper, ctx, userAddr, sdk.NewCoins(
...
func TestNewPoolNotEnoughFunds(t *testing.T) {
	...
	app.SpotKeeper.SetParams(ctx, types.NewParams(
		/*startingPoolNumber=*/ 1,
		/*poolCreationFee=*/ sdk.NewCoins(sdk.NewInt64Coin(denoms.NIBI, 1000*common.TO_MICRO)),
		/*whitelistedAssets*/ []string{},
	))
...
func TestJoinPool(t *testing.T) {
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, testutil.FundAccount(app.BankKeeper, ctx, joinerAddr, tc.joinerInitialFunds))
...
func TestExitPool(t *testing.T) {
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, testutil.FundAccount(app.BankKeeper, ctx, sender, tc.joinerInitialFunds))
			require.NoError(t, testutil.FundAccount(app.BankKeeper, ctx, tc.initialPool.GetAddress(), tc.initialPoolFunds))
