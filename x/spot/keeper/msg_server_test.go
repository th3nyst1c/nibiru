	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.senderInitialFunds != nil {
				require.NoError(t, testutil.FundAccount(app.BankKeeper, ctx, tc.creatorAddr, tc.senderInitialFunds))
			}
...
func TestMsgServerSwapAssets(t *testing.T) {
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// fund user account
			sender := testutil.AccAddress()
			require.NoError(t, testutil.FundAccount(app.BankKeeper, ctx, sender, tc.userInitialFunds))
